...
> **Population**: contains total population counts for each country per year. There are 197 rows/observations (a country) and 302 columns/variables (each year from 1800 to 2100).

>**Murders (total deaths)**: contains total number of estimated deaths from interpersonal violence for each country per year. There are 204 rows/observations (a country) and 31 columns/variables (each year from 1990 to 2019).

>**Happiness Score (WHR)**: contains happiness score for each country per year. It contains the national average response to the question of life evaluations asking the following “Please imagine a ladder, with steps numbered from 0 at the bottom to 10 at the top. The top of the ladder represents the best possible life for you and the bottom of the ladder represents the worst possible life for you. Gapminder has converted it to a 0 to 100 scale so it's in terms of percentage. There are 163 rows/observations (a country) and 19 columns/variables (each year from 2005 to 2022).
...
For this graph, there seems to be a weak, negative association between the overall average murder rate and happiness score. This indicates that as the log-scaled average murder rate increases, the average happiness score tends to decrease. 

There seems to be a moderate negative correlation, as the points are somewhat dispersed around the line, indicating that the murder rate might not be the only factor that influences happiness scores. For different average murder rates (log), there are a lot of high and low average happiness scores. Because of this, there does not appear to be any noticeable extreme outliers or unusual observations, but there is some spread in the data, especially in the range of lower murder rates.
