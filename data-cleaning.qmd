---
title: "data-cleaning"
author: "Thien An Tran", "Tejasree Kandibanda", "Matthew Huang", "Chloe Anbarcioglu"
format: 
  html:
    embed-resources: true
    code-tools: true
    toc: true
editor: source
execute: 
  error: true
  echo: true
  message: false
  warning: false
---

## Setup

```{r setup}
library("tidyverse")
murders <- read_csv("data/murder_total_deaths.csv")
happiness <- read_csv("data/hapiscore_whr.csv")
population <- read_csv("data/pop.csv")
```

## Data Cleaning

Converting Murder Counts to observations
```{r}

murders_clean <- murders
  



```

```{r}
convert_population <- function(population_value) {
  population_value <- as.character(population_value)
  
  multiplier <- case_when(
    str_detect(population_value, "k") ~ 1e3,
    str_detect(population_value, "M") ~ 1e6,
    str_detect(population_value, "B") ~ 1e9,
    TRUE ~ 1
  )
  
  numeric_value <- as.numeric(str_remove_all(population_value, "[kMB]"))
  
  return(numeric_value * multiplier)
}

population_clean <- population |>
  filter(country != "Holy See") |>
  select("country", "1905":"2019") |>
  pivot_longer(cols = "1905":"2019",
               names_to = "Year",
               values_to = "Population") |>
  mutate(across(Population, ~convert_population(.)))

population_clean
```

###Pivoting Longer

###Cleaning

###Joining Datasets
